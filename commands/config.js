...
`);
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "Bạn muốn thay đổi khẩu lệnh thành gì?"
      );
      ...
      client.sendTime(
        message.channel,
        `Đổi khẩu lệnh thành \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel,
        "Vui lòng chọn role bạn muốn đặt làm `DJ's` của bot."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel,
          "Vui lòng tag vai trò mà bạn muốn dành cho DJ."
        );
      ...
      client.sendTime(
        message.channel,
        "Đã thay đổi role cho DJ thành <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Kiểm tra khẩu lệnh của bot",
        type: 1,
        required: false,
        options: [
          {
            name: "symbol",
            description: "Đặt khẩu lệnh cho bot",
            type: 3,
            required: false,
          },
        ],
      ...
      {
        name: "dj",
        description: "Kiểm tra role của DJ",
        type: 1,
        required: false,
        options: [
          {
            name: "role",
            description: "Đặt role cho DJ",
            type: 8,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Khẩu lệnh đã được đặt thành \`${prefix}\``
          );
        } else {
          //has not prefix
          client.sendTime(
            interaction,
            `Khẩu lệnh bot đổi thành \`${GuildDB.prefix}\``
          );
        }
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Đã chuyển role của DJ sang cho  ${role.name}`
          );
        } else {
          /**
          ...
          client.sendTime(
            interaction,
            `Role DJ có thể bật nhạc là ${role.name}`
          );
        }
      }
    },
