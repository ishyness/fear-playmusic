...
module.exports = {
  name: "move",
  description: "Di chuyển tới vị trí nào đó.",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "❌ | **Số bản nhạc không hợp lệ.**");
    }
    ...
    if (dest < 1 || dest > player.queue.length - 1) {
      return client.sendTime(
        message.channel,
        "❌ | **Điểm đến của bản nhạc không hợp lệ.**"
      );
    }
  ...
  SlashCommand: {
    options: [
      ...
      },
      {
        ...
        type: 4,
        required: true,
        description: "Di chuyển bản nhạc đã chọn đến vị trí được chỉ định.",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!args[0].value || !args[1].value)
        return client.sendTime(interaction, "❌ | **Số bản nhạc không hợp lệ.**");
      ...
      client.sendTime(
        ...
        "✅ | **" +
          track.title +
          "** Di chuyển bản nhạc đã chọn đến vị trí được chỉ định " +
          (dest + 1) +
          "."
      );
    },
