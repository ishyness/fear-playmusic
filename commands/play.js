...
module.exports = {
  name: "play",
  description: "Play bài hát của bạn",
  usage: "[tên bài hát hoặc link ]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **Bạn phải ở trong room thoại để bật gì đó!**"
      );
    if (
      ...
      return client.sendTime(
        message.channel,
        "❌ | **Bạn phải ở cùng kênh thoại với BOT mới có thể dùng lệnh!**"
      );
    ...
    if (!player)
      return client.sendTime(
        message.channel,
        "❌ | **Không có bài hát nào đang chạy...**"
      );
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "NO_MATCHES")
          ...
          SongAddedEmbed.addField("Ca sĩ", Searched.tracks[0].author, true);
          SongAddedEmbed.addField(
            "Thời lượng",
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Chạy bài hát ở room thoại",
      },
    ],
    /**
     *
