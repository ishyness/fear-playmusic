...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Cách sử dụng - **\`${GuildDB.prefix}search [nội dung cần tìm]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Không tìm thấy kết quả nào cho " + SearchString
      );
    else {
      ...
      let Pages = songs.map((songz) => {
        ...
        let em = new MessageEmbed()
          .setAuthor(
            "Kết quả tìm kiếm của " + SearchString,
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Bạn đã mất quá nhiều thời gian để trả lời. Chạy lại lệnh nếu bạn muốn chơi thứ gì đó!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Vui lòng nhập số ID bài hát chính xác"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Không tìm thấy bài hát nào cho ID đã chọn."
        );
      player.queue.add(Song);
      if (!player.playing && !player.paused && !player.queue.size)
        player.play();
      let SongAddedEmbed = new MessageEmbed();
      SongAddedEmbed.setAuthor(`Thêm vào hàng chờ`, client.botconfig.IconURL);
      ...
      SongAddedEmbed.addField("Nghệ sĩ", `${Song.author}`, true);
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Nhập tên bài hát hoặc link bạn muốn tìm kiếm",
      },
    ],
    /**
     *
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Được thêm vào hàng đợi:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
        ...
        switch (res.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Được thêm vào hàng chờ:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
          ...
          case "SEARCH_RESULT":
            ...
            try {
              ...
              return awaitchannel.send(
                "❌ | **Bạn đã không cung cấp lựa chọn**"
              );
            }
            ...
            if (first.toLowerCase() === "cancel") {
              if (!player.queue.current) player.destroy();
              return awaitchannel.send("Tìm kiếm bị hủy.");
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Con số bạn đã cung cấp lớn hơn hoặc nhỏ hơn tổng số tìm kiếm. Cách sử dụng - \`(1-${max})\``
              );
            ...
            if (!player.playing && !player.paused && !player.queue.length) {
              ...
              SongAddedEmbed.setAuthor(
                `Được thêm vào hàng chờ`,
                client.botconfig.IconURL
              );
              SongAddedEmbed.setThumbnail(track.displayThumbnail());
              SongAddedEmbed.setColor(client.botconfig.EmbedColor);
              SongAddedEmbed.setDescription(`[${track.title}](${track.uri})`);
              SongAddedEmbed.addField("Nghệ sĩ", track.author, true);
              SongAddedEmbed.addField(
                "Duration",
                `\`${prettyMilliseconds(track.duration, {
                  colonNotation: true,
